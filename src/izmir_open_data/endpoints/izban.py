...
class IzbanDurakMesafesi(BaseModel):
    ...
    istasyon_sirasi: int | str = Field(
        alias="ISTAYON_SIRASI", description="Hat üzerindeki istasyon sırası"
    )
    mesafe: float | str = Field(
        alias="MESAFE", description="Bir önceki istasyona olan mesafe (metre cinsinden)"
    )


class IzbanEndpoint(BaseEndpoint):
    async def get_tarife(
        self, binis_istasyonu_id: int, inis_istasyonu_id: int, aktarma: int, htt_mi: int
    ) -> IzbanUcretDetay:
        """
        ...
        return await self._client.get(
            f"izban/tutarhesaplama/{binis_istasyonu_id}/{inis_istasyonu_id}/{aktarma}/{htt_mi}"
        )
    ...
    async def get_hareket_saatleri(
        self, kalkis_istasyon_id: int, varis_istasyon_id: int
    ) -> Any:
        """
